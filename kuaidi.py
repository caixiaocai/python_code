...
def searchPackageBSHT():
    packageNum = input('请输入快递号：')
    # url的寻找，优先找XHR，找到对应的接口
    url1 = 'http://www.kuaidi100.com/autonumber/autoComNum?resultv2=1&text=' + packageNum
    companyName = json.loads(requests.get(url1).text)['auto'][0]['comCode']
    print(companyName)
    # 两种格式化的方式实现
    # url2 = 'https://www.kuaidi100.com/query?type={}&postid={}&temp=0.5355317528382457'.format(companyName, packageNum)
    url2 = 'https://www.kuaidi100.com/query?type=%s&postid=%s&temp=0.5355317528382457' % (companyName, packageNum)
    for item in json.loads(requests.get(url2).text)['data']:
        print(item['time'], item['context'])
searchPackageBSHT()
